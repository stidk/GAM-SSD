...
__all__ = ['SSD_global_se2']


@register
class SSD_global_se2(object):
    """
    ...
    def __init__(self,
                 ...
                 num_classes=21):
        super(SSD_global_se2, self).__init__()
    ...
    def build(self, feed_vars, mode='train'):
        ...
        for body_feat in body_feats1:
            mixed_precision_enabled = mixed_precision_global_state() is not None
            body_feats_w.append(fluid.layers.pool2d(
                input=body_feat,
                pool_size=0,
                pool_type='avg',
                global_pooling=True,
        ...
        body_feats_all_w = fluid.layers.concat([body_feats_w[0], body_feats_w[1], body_feats_w[2], body_feats_w[3]], axis=1)
        body_feats_all_w2 = fluid.layers.fc(
            input=body_feats_all_w,
            size=body_feats_all_w.shape[1],
            act='sigmoid' )

        for i in range(len(num_of_layers)):
            body_feats_w2 = fluid.layers.fc(
                input=body_feats_all_w2,
                size=num_of_layers[i],
                act='sigmoid')
            body_feats2 += [fluid.layers.elementwise_mul(x=body_feats1[i], y=body_feats_w2, axis=0)]

        # body_feats2 = body_feats1 
        ...
        #     del(new_feats[1])
        #     print(new_feats)
        body_feats3 = OrderedDict([('new_feats{}'.format(idx),feat)
                                for idx, feat in enumerate(body_feats2)])
        if self.fpn is not None:
            body_feats3, spatial_scale = self.fpn.get_output(body_feats3)
        ...
        if isinstance(body_feats3, OrderedDict):
            ...
            #  new_feats[0] = fluid.layers.concat([new_feats_change, new_feats[1]], axis = 1)
             new_feats_change2 = self.BasicConv(body_feats[1], 256, filter_size = 1)
            #  print(new_feats_change, new_feats_change2)
            #  new_feats_change3 = self.BasicConv(body_feats[1], 256, filter_size = 1, down_size = 32)
            #  new_feats_change4 = self.BasicConv(body_feats[2], 256, filter_size = 1)
             new_feats[0] = fluid.layers.concat([new_feats_change, new_feats_change2], axis = 1)
            #  t = new_feats_change + new_feats_change2
            #  t= fluid.layers.conv2d(input=t,
            #                     num_filters=256,
            #                     filter_size=3,
            #                     padding=1,
            #                     act=None,
            #                     )
            #  new_feats[0] = t
            #  new_feats[1] = fluid.layers.concat([new_feats_change3, new_feats_change4], axis = 1)
             del(new_feats[1])
        ...
        #body_feat_names = list(reversed(list(body_feats3.keys())))
        #body_feats = [body_feats3[name] for name in body_feat_names]
        # body_feats = body_feats[0:4]
        # body_feats[0] = fluid.layers.interpolate(body_feats[0], out_shape =(38, 38), resample='BILINEAR', align_corners=True)
        # body_feats[1] = fluid.layers.interpolate(body_feats[1], out_shape =(19, 19), resample='BILINEAR', align_corners=True)
        # body_feats[2] = fluid.layers.interpolate(body_feats[2], out_shape =(10, 10), resample='BILINEAR', align_corners=True)
        # body_feats[3] = fluid.layers.interpolate(body_feats[3], out_shape =(5, 5), resample='BILINEAR', align_corners=True)
        # # body_feats.append(fluid.layers.interpolate(body_feats2[0], out_shape =(5, 5), resample='BILINEAR', align_corners=True))
        # body_feats.append(fluid.layers.interpolate(body_feats2[1], out_shape =(3, 3), resample='BILINEAR', align_corners=True))
        # body_feats.append(fluid.layers.interpolate(body_feats2[2], out_shape =(1, 1), resample='BILINEAR', align_corners=True))

        # del(body_feats[1])
        del(body_feats[-2])
        print('body_feats',body_feats)
    ...
    def _inputs_def(self, image_shape):
        ...
        # yapf: enable
        return inputs_def
    def BasicConv(self, input, num_filters, filter_size, stride = 1, padding = 0, dilation = 1, act = 'relu', bn = True, down_size = 0, groups = 1):
        # if down_size > 0:            
        #     input = fluid.layers.interpolate(input, out_shape =(down_size, down_size), resample='BILINEAR', align_corners=True)         
        ...
        if down_size > 0:            
            conv_1 = fluid.layers.interpolate(conv_1, out_shape =(down_size, down_size), resample='NEAREST', align_corners=True)         
        if act =='relu':
            conv_1 = fluid.layers.relu(conv_1)
        # if down_size > 0:            
